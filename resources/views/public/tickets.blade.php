...
<head>
    ...
    <title>{{ $event->name }} - Ticket Selection</title>
...
<body class="bg-gray-50 min-h-screen">
    ...
    <header class="bg-white shadow-sm border-b">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            <div class="flex items-center justify-between">
                ...
                <a href="{{ route('public.events') }}" class="text-blue-600 hover:text-blue-800 text-sm font-medium">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                @foreach($zones as $zone)
                    <div class="bg-white rounded-lg p-4 border border-blue-200">
                        <h4 class="font-medium text-gray-900 mb-2">{{ $zone->name }}</h4>
                        @if($zone->description)
        ...
        <div class="space-y-8">
            @foreach($zones as $zone)
                @if(isset($ticketsByZone[$zone->id]) && $ticketsByZone[$zone->id]->count() > 0)
                    <div class="bg-white rounded-lg shadow-sm border">
                        ...
                        <div class="bg-gradient-to-r from-gray-50 to-gray-100 px-6 py-4 border-b">
                            ...
                            @if($zone->description)
                                <p class="text-gray-600 mt-1">{{ $zone->description }}</p>
                            @endif
                        </div>
                        ...
                        <div class="p-6">
                            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                @foreach($ticketsByZone[$zone->id] as $ticket)
                                    <div class="border border-gray-200 rounded-lg p-4 hover:border-blue-300 hover:shadow-md transition duration-200">
                                        <div class="flex justify-between items-start mb-3">
                                            ...
                                            <div class="text-right">
                                                ...
                                                <div class="text-xs text-gray-500">{{ $ticket->currency->code ?? 'USD' }}</div>
                                            </div>
                                        </div>

                                        @if($ticket->info)
                                            <p class="text-sm text-gray-600 mb-4">{{ $ticket->info }}</p>
                                        @endif
                                        ...
                                        <!-- Register Button -->
                                        <a href="{{ route('public.form', $ticket->id) }}" 
