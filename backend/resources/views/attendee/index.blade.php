...
@section('content')
        ...
        <div class="max-w-6xl mx-auto px-4 py-6">
            <div class="flex justify-between items-center">
                ...
                <div class="flex space-x-3">
                    <a href="{{ route('attendees.export') }}{{ request()->has('event_id') ? '?event_id=' . request('event_id') : '' }}" 
                       class="px-4 py-2 bg-green-600 hover:bg-green-700 text-white rounded-md transition duration-200 flex items-center">
                        <i class="fas fa-download mr-2"></i>Export CSV
                    </a>
                    <a href="{{ route('attendees.create') }}" class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-md transition duration-200 flex items-center">
    ...
    <div class="max-w-6xl mx-auto px-4 py-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <div>
                            <select id="filterEvent" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                ...
                                @foreach($attendees->unique('event.id') as $attendee)
                                    <option value="{{ $attendee->event->name }}">{{ $attendee->event->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                    ...
                    <div class="border-t border-gray-200 mt-4 pt-4">
                        <div class="flex justify-between items-center">
                            <div class="text-sm text-gray-600">
                                <span id="visibleCount">{{ $attendees->count() }}</span> attendees displayed
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-3xl font-bold text-gray-900">{{ $attendees->count() }}</p>
                        </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-3xl font-bold text-green-900">{{ $attendees->where('checked_in', true)->count() }}</p>
                        </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-3xl font-bold text-orange-900">{{ $attendees->where('checked_in', false)->count() }}</p>
                        </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-3xl font-bold text-purple-900">${{ number_format($attendees->sum(function($attendee) { return $attendee->payments->sum('amount'); }), 2) }}</p>
                        </div>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            ...
                            <tr class="attendee-row hover:bg-gray-50">
                                    ...
                                    <div class="flex items-center">
                                        <div class="flex-shrink-0 h-10 w-10">
                                            <div class="h-10 w-10 rounded-full bg-blue-500 flex items-center justify-center">
                                                <span class="text-white font-medium">{{ substr($attendee->first_name, 0, 1) }}{{ substr($attendee->last_name, 0, 1) }}</span>
                                            </div>
                                        </div>
                                        <div class="ml-4">
                                            ...
                                            @if($attendee->phone)
                                                <div class="text-xs text-gray-400">{{ $attendee->phone }}</div>
                                            @endif
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900">{{ $attendee->event->name }}</div>
                                    <div class="text-sm text-gray-500">{{ $attendee->event->event_date->format('M j, Y') }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="text-sm font-medium text-gray-900">{{ $attendee->ticket->name }}</div>
                                    <div class="text-sm text-gray-500">${{ number_format($attendee->ticket->price, 2) }}</div>
                                    <div class="text-xs text-gray-400">{{ $attendee->ticket->eventZone->name ?? 'N/A' }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if($attendee->company)
                                        <div class="text-sm text-gray-900">{{ $attendee->company }}</div>
                                        @if($attendee->job_title)
                                            <div class="text-sm text-gray-500">{{ $attendee->job_title }}</div>
                                        @endif
                                    @else
                                        <span class="text-gray-400">-</span>
                                    @endif
                                    @if($attendee->country)
                                        <div class="text-xs text-gray-400">{{ $attendee->country }}</div>
                                    @endif
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if($attendee->checked_in)
                                        ...
                                        @if($attendee->checked_in_at)
                                            <div class="text-xs text-gray-500 mt-1">{{ $attendee->checked_in_at->format('M j, g:i A') }}</div>
                                        @endif
                                    @else
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                    <div class="flex space-x-2">
                                        <a href="{{ route('attendees.show', $attendee->id) }}" class="text-blue-600 hover:text-blue-900">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <a href="{{ route('attendees.edit', $attendee->id) }}" class="text-indigo-600 hover:text-indigo-900">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        @if(!$attendee->checked_in)
                                            <button onclick="checkIn({{ $attendee->id }})" class="text-green-600 hover:text-green-900" title="Check In">
                                                <i class="fas fa-check-circle"></i>
                                            </button>
                                        @endif
                                        <form action="{{ route('attendees.destroy', $attendee->id) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this attendee?')">
                                            @csrf
                                            @method('DELETE')
        ...
        @else
            ...
            <div class="bg-white rounded-lg shadow-md p-12 text-center">
                ...
                <a href="{{ route('attendees.create') }}" class="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-md transition duration-200">
    ...
    <script>
        ...
        function exportFiltered() {
            ...
            let exportUrl = '{{ route("attendees.export") }}?';
